    ...
    case TYPE_OP:
        ...
        case VAR:
            if (signal) {       /* global var declaration */
                if (p->op.num == 2) {   /*scalar*/
                    code_declare_global_var(p->op.node[1]->index, 1);
                    s_or_a = hash_insert(var_global_SorA, HASHSIZE, p->op.node[1]->index, SCALAR);
                } else if (p->op.num == 3) {    /* array */
                    code_declare_global_var(p->op.node[1]->index, p->op.node[2]->content);
                    s_or_a = hash_insert(var_global_SorA, HASHSIZE, p->op.node[1]->index, ARRAY);
                }
            } else {
        ...
        case FUNC:
            ...
            if (!retvalue) {    /* start text section */
                ...
                //make sure that input and output can be used.
                //actually this should depend on whether the input or output used or not.
                /* output function */
        ...
        case '/':
            strcpy(binary_op_str, "/");
          op_binary:count = exeNode(p->op.node[0], 1);
            count += exeNode(p->op.node[1], 1);
            code_pop(2);
            code_pop(1);
